...
const shareUrl = () => `${window.location.origin}${BIRTHDAY.share.link}`
...
function useAmbient() {
  ...
  const toggle = () => {
    ...
    master.gain.setTargetAtTime(0.04, ctx.currentTime, 0.5)
    master.connect(ctx.destination)

    // Cheerful major chord: C major with higher octave
    ...
    chord.forEach((f, i) => {
      ...
      gain.gain.value = 0.4 / chord.length
      osc.connect(gain)
      gain.connect(master)
      osc.start()
    })
...
export function MusicToggle() {
  ...
  return (
    ...
    >
      ...
      <span className="hidden max-w-[180px] truncate text-xs text-[var(--exp-muted)] lg:block">{playing ? BIRTHDAY.music.track : 'Play music'}</span>
    </button>
  )
}
...
function ShareModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const downloadQr = () => {
    ...
    a.href = url
    a.download = 'aarav-birthday-qr.svg'
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <h3 className="mt-2 font-display text-2xl font-light">{BIRTHDAY.share.title}</h3>
            <p className="mt-1.5 text-sm text-[var(--exp-muted)]">{BIRTHDAY.share.subtitle}</p>

            <div className="mt-5 flex justify-center rounded-2xl bg-white p-4">
              <div ref={qrRef} className="h-40 w-40">
                <QRCodeSVG value={shareUrl()} size={160} level="M" fgColor="#1a2744" bgColor="#ffffff" />
              </div>
            </div>
            ...
            <a
              href={`https://wa.me/?text=${encodeURIComponent(`${BIRTHDAY.share.whatsappText}\n${shareUrl()}`)}`}
              target="_blank"
              rel="noreferrer"
...
export function BirthdayFloating() {
  return (
    <>
      <MusicToggle />
      <ShareButton />
...
function MobileBarWrapper() {
  ...
  return <MobileBar visible={visible} />
}
