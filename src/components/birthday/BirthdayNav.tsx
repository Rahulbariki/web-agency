...
const LINKS = [
  ...
  { label: 'Celebration', href: '#party' },
  { label: 'Memories', href: '#gallery' },
  { label: 'Venue', href: '#venue' },
  { label: 'RSVP', href: '#rsvp' },
]

export function BirthdayNav({ scrolled, onClose }: { scrolled: boolean; onClose: () => void }) {
    ...
    <>
      ...
      >
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5 sm:px-8">
          ...
          >
            <span
              className="flex h-8 w-8 items-center justify-center rounded-full border text-[0.6rem] font-semibold tracking-widest"
              style={{ borderColor: 'color-mix(in srgb, var(--exp-a) 55%, transparent)' }}
            >
              A·1
            </span>
            <span className="hidden font-display text-sm font-medium uppercase tracking-[0.25em] sm:inline">
              Aarav
            </span>
          </button>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <p className="mt-10 text-center text-[0.6rem] uppercase tracking-[0.4em] text-[var(--exp-muted)]">{BIRTHDAY.dateShort} · {BIRTHDAY.city}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </>
