...
export function WeddingStory({ demo }: { demo: ExperienceDemo }) {
  return (
    <section id="story" className="relative scroll-mt-20">
      <div className="mx-auto max-w-6xl px-5 py-24 sm:px-8 sm:py-32">
        <Fade className="text-center">
          <p className="text-[0.65rem] font-semibold uppercase tracking-[0.4em] text-[var(--exp-muted)]">{WEDDING.monogram}</p>
          <h2 className="mt-4 font-display text-4xl font-light italic sm:text-5xl">{WEDDING.story.heading}</h2>
        ...
        <div className="relative mt-24">
          ...
          <div className="space-y-16 sm:space-y-24">
            {WEDDING.story.timeline.map((item, i) => {
              const flip = i % 2 === 1
              return (
                <div key={item.year} className="relative grid gap-8 md:grid-cols-2 md:gap-20">
                  <span
                  ...
                  <Fade className={cn('pl-14 pt-2 md:pl-0 md:pt-2', flip ? 'md:order-2' : 'md:order-1')}>
                    <p className="text-[0.6rem] font-semibold uppercase tracking-[0.35em] text-[var(--exp-a)]">{item.year}</p>
                  ...
                  <Fade className={cn('pl-14 md:pl-0', flip ? 'md:order-1' : 'md:order-2')}>
                    <DemoArt
                      ...
                      c={demo.theme.c}
                      className={cn(
                        'aspect-[4/3] w-full rounded-2xl border border-[var(--exp-border)] shadow-[0_24px_60px_-30px_rgba(0,0,0,0.35)]',
                      )}
                    />
                  </Fade>
                </div>
