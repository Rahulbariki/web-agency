...
export function WeddingGallery({ demo }: { demo: ExperienceDemo }) {
  const [index, setIndex] = useState<number | null>(null)
  const close = useCallback(() => setIndex(null), [])
  const step = useCallback((dir: 1 | -1) => {
    setIndex((i) => (i === null ? i : (i + dir + WEDDING.gallery.length) % WEDDING.gallery.length))
  }, [])
    ...
    <section id="gallery" className="scroll-mt-20">
      <div className="mx-auto max-w-6xl px-5 py-24 sm:px-8 sm:py-32">
        <Fade className="text-center">
          <p className="text-[0.65rem] font-semibold uppercase tracking-[0.4em] text-[var(--exp-muted)]">Moments</p>
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-[var(--exp-muted)]">
            A few frames from our story so far — tap any photo to view it up close.
          </p>
      ...
      <AnimatePresence>
        {index !== null && (
          <motion.div
            ...
            onClick={close}
          >
            <div className="absolute left-4 top-4 text-[0.65rem] font-semibold uppercase tracking-[0.3em] text-[#c9a968]">
              {index + 1} / {WEDDING.gallery.length}
            </div>
            <button
              onClick={close}
            ...
            >
              <DemoArt
                scene={WEDDING.gallery[index].scene}
                c={demo.theme.c}
                animated={false}
                className="aspect-[4/3] max-h-[68vh] w-full rounded-xl border border-white/10 shadow-2xl"
              />
              <p className="mt-5 text-center font-display text-lg italic text-[#e8d6b4]">{WEDDING.gallery[index].caption}</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
