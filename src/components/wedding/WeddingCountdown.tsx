...
export function WeddingCountdown() {
  const target = useMemo(() => new Date(WEDDING.target), [])
    ...
    <section id="countdown" className="relative scroll-mt-24 border-b border-[var(--exp-border)] bg-[var(--exp-bg-2)]">
      <div className="mx-auto max-w-6xl px-5 py-20 text-center sm:px-8 sm:py-24">
        <Fade>
          ...
          <h2 className="mt-4 font-display text-4xl font-light italic sm:text-5xl">Counting down to forever.</h2>
          <p className="mt-3 text-sm text-[var(--exp-muted)]">{WEDDING.dateDisplay} · {WEDDING.venue}, {WEDDING.city}</p>
        </Fade>
